...
mongo = PyMongo(app,uri="mongodb://localhost:27017/test")  # 開啟database
@app.route('/query/<string:event_id>')
def query_user(event_id):
    if event_id:
        users = mongo.db.test_case.find({'event_id': event_id})
        x=[]
        if users:
            for i in users:
                i.pop("_id")
...
def post_data(person):
    ...
    mongo.db.test_case.insert_one(personobj)
    return str(person)
...
def delete_docs(event_id):
    mongo.db.test_case.remove({"event_id":event_id})
    return "Delete success"
@app.route('/update/<string:event_id>')
def update_docs(event_id):
    doc=mongo.db.test_case.update(
        {"event_id" : event_id},
        {"$set":
            {"status": "green"}
        },upsert=True)
